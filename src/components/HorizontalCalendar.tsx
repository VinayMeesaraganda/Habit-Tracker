...
export function HorizontalCalendar({ currentMonth, selectedDate, onSelectDate }: HorizontalCalendarProps) {
    ...
    return (
        <div className="w-full bg-[#0A0A0B] border-b border-white/5 sticky top-0 z-30">
            <div
                ref={scrollRef}
                className="flex overflow-x-auto gap-2 p-2 no-scrollbar items-center px-3 snap-x scrollbar-dark"
                ...
                {monthDays.map(day => {
                    ...
                    return (
                        <button
                            ...
                            className={`
                                flex flex-col items-center justify-center min-w-[48px] h-[56px] rounded-xl transition-all duration-300 snap-center
                                ${isSelected
                            ...
                            `}
                        >
                            <span className={`text-[9px] font-bold uppercase tracking-wider mb-0.5 ${isSelected ? 'text-white/70' : 'text-gray-500'}`}>
                                {format(day, 'EEE')}
                            </span>
                            ...
                            {isCurrentDay && !isSelected && (
                                <div className="mt-0.5 w-1 h-1 rounded-full bg-primary-500" />
                            )}
                        </button>
                    );
                })}
